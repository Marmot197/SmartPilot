...
# Define example prompts for each intent
intent_examples = {
    "qa_agent": ["What is this system about?", "Can you explain the results?", "Help me understand this better."],
    "doc_agent": ["Where is this documented?", "Show me the documentation.", "Explain the process using docs."],
    "casual_agent": ["How are you?", "What do you like to do?", "Tell me something interesting."],
    "forecasting_agent": ["What will happen next?", "Give me a forecast.", "Predict the future trend."]
}

# Encode intent examples
encoded_intents = {k: intent_model.encode(v, convert_to_tensor=True) for k, v in intent_examples.items()}

def classify_intent(user_input):
    input_embedding = intent_model.encode(user_input, convert_to_tensor=True)
    best_score = -1
    best_intent = "qa_agent"
    for intent, examples in encoded_intents.items():
        similarity = util.pytorch_cos_sim(input_embedding, examples).mean().item()
        if similarity > best_score:
            best_score = similarity
            best_intent = intent
    return best_intent

def route_input(user_input, current_agent="qa_agent"):
    new_agent = classify_intent(user_input)
    return new_agent if new_agent != current_agent else current_agent

# Example agent interface
def qa_agent(user_input):
    # Replace with actual QA logic
    response = "QA Agent: " + user_input
    next_agent = route_input(user_input, current_agent="qa_agent")
    return response, next_agent

def doc_agent(user_input):
    response = "Documentation Agent: " + user_input
    next_agent = route_input(user_input, current_agent="doc_agent")
    return response, next_agent

def casual_agent(user_input):
    response = "Casual Agent: " + user_input
    next_agent = route_input(user_input, current_agent="casual_agent")
    return response, next_agent

def forecasting_agent(user_input):
    response = "Forecasting Agent: " + user_input
    next_agent = route_input(user_input, current_agent="forecasting_agent")
    return response, next_agent
...
def interact(user_input, current_agent="qa_agent"):
    response, next_agent = agent_map[current_agent](user_input)
    if next_agent != current_agent:
        return interact(user_input, next_agent)
    return response
...
if user_input:
    ...
    # 🟣 Production forecasting
        ...
        if process_response is None:
            ...
            llm = LLM()
            llm.set_prompt(AssetLoader.get_templates().get("QA", ""), user_input, combined_context)

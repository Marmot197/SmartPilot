...
def process_user_input(user_input, state):
    ...
    if state.get("ProcessOntologyQa") is None:
        state["ProcessOntologyQa"] = ProcessOntologyQA(
        "assets/d3_graph.json")
    ...
    if user_input:
        ...
        # 🟠 Root Cause Analysis Queries (semantic match)
                ...
                and "rca_results" in state
                and state["rca_results"]
        ):
            response = answer_root_cause_query(user_input, state["rca_results"])

        # 🟩 Causal yes/no relation
        elif any(kw in user_query_lower for kw in
                ["does", "cause", "causal relation"]) and "causal_relations" in state:
            response = answer_causal_query(user_input, state["causal_relations"])
        ...
        elif state.get("ProcessOntologyQa") is not None:
            process_qa = state["ProcessOntologyQa"]
            ...
            if process_response is None:
                ...
                try:
                    ...
                    if "rate_limit_exceeded" in str(e).lower() or "request too large" in str(e).lower():
                        # print("⚠️ Trimming context and retrying due to token rate limit...")
                        state["context"] = state["context"][-2000:]
                ...
                if topic == "Toy rocket":
                    ...
                    if "selected_features" in state:
                        kg_descriptions = []
                        for feature in state["selected_features"].split(","):
                    ...
                    if "ProcessOntologyQa" in state:
                        process_qa = state["ProcessOntologyQa"]
                        ontology_data = process_qa.get_ontology_data()
                        process_ontology_info = []
                        for feature in state.get("selected_features", "").split(","):
                    ...
                    if "selected_features" in state and total_effects is not None and node_labels is not None:
                        causal_insights = []
                        selected = [f.strip() for f in state["selected_features"].split(",") if
                    ...
                    state["context"] = enrich_context_with_memory(state["context"], user_input)
                    ...
                    recent_msgs = state["messages"][-MAX_CONTEXT_MESSAGES:]
                    ...
                    if len(state["context"]) > 3000:
                        state["context"] = state["context"][-3000:]
            ...
            if match:
                ...
                if state["uploaded_data"] is not None:
                    ...
                    if feature_to_add in available_columns:
                        current_features = [f.strip() for f in state.get("selected_features", "").split(",") if
                                            f.strip()]

                        if feature_to_add not in current_features:
                            current_features.append(feature_to_add)
                            state["selected_features"] = ", ".join(current_features)
                            ...
                            # Note: You'll need to modify run_lingam() to work with state dict too
                            # run_lingam()
                            save_event_to_memory("causal_discovery", {
                                "graph": "lingam_causal_graph.html",
                                "features": state.get("selected_features", ""),
                                "dataset": state.get("uploaded_file_path", "")
                            })
...
def main(user_input):
      ...
      state = {
        ...
        "causal_relations": [],
        "rca_results": None,
        "causal_eval_results": None,
        "feature_descriptions": "",
        "context": "",
        "robot_label_to_uuid": {},
        "uuid_to_robot_label": {},
        "ProcessOntologyQa": None,
        "bootstrap_results_df": None
      }
      st.session_state = state
      response = process_user_input(user_input, state)
    #   print(response)  # or return response
      return response

main("List the 5 capabilities of the causal analysis agent.")